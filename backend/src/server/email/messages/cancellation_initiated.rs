    ...
    fn body_html(&self) -> String {
        ...
        let (content, cta_href) = if self.licensed {
            (
                content
                    .paragraph(&format!(
                        "Your Scanopy license subscription is scheduled to cancel on <strong>{}</strong>. Your license key keeps working until then, after which your server stops accepting it and goes read-only.",
                        self.period_end
                    ))
