...
/// `sub` claim of offline keys and of the entitlements the cloud returns for
/// online keys.
pub const OFFLINE_KEY_SUBJECT: &str = "scanopy-license";
...
impl LicenseKey {
    ...
    pub fn key_type(&self) -> LicenseKeyType {
        match jsonwebtoken::decode::<OnlineKeyClaims>(
            &self.0,
            &decoding_key(),
            &validation(&["sub", "iss", "iat"]),
        ) {
            Ok(token_data) if token_data.claims.sub == ONLINE_KEY_SUBJECT => {
                LicenseKeyType::Online(token_data.claims)
            }
            _ => LicenseKeyType::Offline,
        }
    }
    ...
    pub fn validate(&self) -> LicenseStatus {
        match jsonwebtoken::decode::<LicenseClaims>(
            ...
            Ok(token_data) => {
                if token_data.claims.sub != OFFLINE_KEY_SUBJECT {
...
pub(crate) mod test_support {
    ...
    use super::{LicenseKey, OFFLINE_KEY_SUBJECT};
    ...
    pub fn license_token(
        ...
        sign(&LicenseClaims {
            sub: OFFLINE_KEY_SUBJECT.to_string(),
            iss: "scanopy".to_string(),
            iat: now,
